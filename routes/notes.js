...
//post route to create a new note in the mongo
router.post('/createNote', function (req, res) {
    let { title, body, movieId } = req.body;
    let note = {
        title,
        body
    }
    db.Note
        ...
        .then(result => {
            db.Movies
                .findOneAndUpdate({ _id: movieId }, { $push: { notes: result._id } }, { new: true })//saving note reference
